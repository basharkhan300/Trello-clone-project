...
class CheckList extends Component {
    ...
    fetchCheckList = async (cardId) => {
        console.log("checklist in cardid ", cardId);
    ...
    componentDidMount(){
        console.log("is mount call");
        console.log("post id is",this.props.id);
    ...
    render() { 
        return ( 
            ...
            this.state.checkList.map((check) => 
                ...
                <h4>{check.name}</h4>
                <button type="submit" className="btn btn-sm m-2 btn-warning" onClick={() => this.deleteChecklist(check.id)}>
              Delete CheckList
            </button>
                </div>
